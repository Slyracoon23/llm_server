...
from models.schemas.discord.entity.discord import DiscordChannel, DiscordMessage, DiscordMember

class DiscordSupportTicketInput(BaseModel):
    channel: DiscordChannel = Field(..., description="The Discord channel of the support ticket")
    messages: List[DiscordMessage] = Field(..., description="List of messages in the support ticket channel")
    members: List[DiscordMember] = Field(..., description="List of Discord members involved in the support ticket")

class DiscordSupportTicketOutput(BaseModel):
...
class DiscordSupportTicketPMQuestionTask(BaseTask):
    name = "generate_pm_questions_for_support_ticket"
    prompt_template = """
    ...
    {{ members | selectattr("id", "eq", message.author_id) | map(attribute="name") | first }} ({{ message.timestamp }}): {{ message.content }}
    {% endfor %}
    ...
    input_schema = DiscordSupportTicketInput
    output_schema = DiscordSupportTicketOutput