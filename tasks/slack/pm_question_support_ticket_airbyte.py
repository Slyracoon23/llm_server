...
from models.schemas.slack.entity.slack import SlackThread, SlackMessage

class SlackSupportTicketInputAirbyte(BaseModel):
    thread_messages: List[SlackThread] = Field(..., description="The list of Slack messages for the support ticket thread")

class SlackSupportTicketOutputAirbyte(BaseModel):
...
class SlackSupportTicketPMQuestionTaskAirbyte(BaseTask):
    name = "generate_pm_questions_for_support_ticket_airbyte"
    prompt_template = """
    You are an AI assistant for Slack, tasked with analyzing support ticket threads. Your role is to generate a list of questions that a product manager would ask about this support ticket to gain actionable insights for product development and improvement.
    ...
    Thread TS: {{ thread_messages[0].ts }}
    Channel ID: {{ thread_messages[0].channel_id }}
    Messages:
    ...
    input_schema = SlackSupportTicketInputAirbyte
    output_schema = SlackSupportTicketOutputAirbyte